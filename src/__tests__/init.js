...
let define = (options = {}) => {
  ...
  User.init(
    ...
    { ...options, sequelize, modelName: "User" }
  );
...
describe("Using Init", () => {
  it("should be possible to reate a user", async () => {
    let { User } = define();
  ...
  it("should work with addHook", async () => {
    let { User } = define();
    ...
    User.addHook("afterFind", (findResult) => {
      ...
      for (const instance of findResult) {
        delete instance.age;
        delete instance.dataValues.age;
      }
    });

    let users = await User.findAll({});

    // expect AGE to not be present in the snapshot
    expect(users).toMatchSnapshot();
  });
});
